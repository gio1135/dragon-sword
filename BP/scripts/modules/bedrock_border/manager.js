...
import { getDimensionKey, hasBypassPermission, findSafeY } from './utils.js';
...
class WorldBorderManagerClass {
  ...
  registerPlayerCleanup() {
    world.afterEvents.playerLeave.subscribe((event) => {
      this.events.emit(BorderEvents.PLAYER_CLEANUP, {
        ...
        for (const dim in this.config) {
          if (parsedConfig[dim]) {
            ...
            if (parsedConfig[dim].size && !parsedConfig[dim].sizeChunks) {
              this.config[dim].sizeChunks = Math.ceil(
                parsedConfig[dim].size / CHUNK_SIZE,
              );
  ...
  saveConfig() {
    try {
      const oldConfig = JSON.parse(
        world.getDynamicProperty('worldBorderConfig') ||
          JSON.stringify(DEFAULT_DIMENSION_CONFIG),
      );
  ...
  checkPlayerPosition(player) {
    ...
    if (outside) {
      if (hasBypass) {
        ...
        this.events.emit(BorderEvents.PLAYER_CROSS_BORDER, {
          player,
  ...
  teleportPlayerBack(player, currentLocation, borderConfig, minSafe, maxSafe) {
    ...
    try {
      player.teleport(
        { x: newX, y: safeY, z: newZ },
        {
          dimension: player.dimension,
        },
      );
      player.playSound(SOUNDS.BORDER_HIT, { volume: 0.5, pitch: 0.8 });
      this.events.emit(BorderEvents.PLAYER_TELEPORTED, {
        player,
        from: currentLocation,
        to: { x: newX, y: safeY, z: newZ },
      });
    } catch (error) {}
  }
