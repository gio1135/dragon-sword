...
class PhysicalBorderManager {
  ...
  updateNearPlayer(player, borderConfig) {
    if (!borderConfig.enabled || player.dimension.id === 'minecraft:the_end')
      return;
    ...
    for (
      let x = playerChunkX - chunksToCheck;
      x <= playerChunkX + chunksToCheck;
      x++
    ) {
      for (
        let z = playerChunkZ - chunksToCheck;
        z <= playerChunkZ + chunksToCheck;
        z++
      ) {
        const chunkKey = `${x},${z}`;
  ...
  generateBorderInChunk(
    ...
    if (minX >= xStart && minX <= xEnd) {
      ...
      if (zLineStart <= zLineEnd) {
        dimension.runCommandAsync(
          `fill ${minX} ${y} ${zLineStart} ${minX} ${y} ${zLineEnd} border_block replace bedrock`,
        );
      }
    }
    ...
    if (maxX >= xStart && maxX <= xEnd) {
      ...
      if (zLineStart <= zLineEnd) {
        dimension.runCommandAsync(
          `fill ${maxX} ${y} ${zLineStart} ${maxX} ${y} ${zLineEnd} border_block replace bedrock`,
        );
      }
    }
    ...
    if (minZ >= zStart && minZ <= zEnd) {
      ...
      if (xLineStart <= xLineEnd) {
        dimension.runCommandAsync(
          `fill ${xLineStart} ${y} ${minZ} ${xLineEnd} ${y} ${minZ} border_block replace bedrock`,
        );
      }
    }
    ...
    if (maxZ >= zStart && maxZ <= zEnd) {
      ...
      if (xLineStart <= xLineEnd) {
        dimension.runCommandAsync(
          `fill ${xLineStart} ${y} ${maxZ} ${xLineEnd} ${y} ${maxZ} border_block replace bedrock`,
        );
      }
    }
  ...
  checkAndCleanupDimension(dimId, oldConf, newConf) {
    ...
    if (
      oldConf.sizeChunks !== newConf.sizeChunks ||
  ...
  scheduleCleanup(dimKey, oldConf) {
    ...
    DS.log(
      `Scheduled border cleanup for ${dimKey} (${chunksToClean.length} chunks)`,
    );
  }
  ...
  runMigration(config) {
    ['overworld', 'nether', 'end'].forEach((dim) => {
      ...
      this.runSpecificLineCleanupManual(
        actualDimId,
        minX1,
        maxX1,
        minX1,
  ...
  processChunkQueue(dimId, queue) {
    ...
    try {
      ...
      system.run(() => {
        try {
          dimension.runCommandAsync(
            `tickingarea add ${cx * 16} 0 ${cz * 16} ${cx * 16 + 15} 0 ${cz * 16 + 15} ${areaName}`,
          );
        } catch (e) {}
        ...
        system.run(() => {
          try {
            rects.forEach((r) => {
              dimension.runCommandAsync(
                `fill ${r.xMin} ${y} ${r.zMin} ${r.xMax} ${y} ${r.zMax} bedrock replace border_block`,
              );
            });
          } catch (e) {}
