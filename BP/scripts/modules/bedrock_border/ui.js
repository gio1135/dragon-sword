...
import { COLORS, CHUNK_SIZE, DEFAULTS } from './constants.js';
...
export const BedrockBorderUI = {
  openMenu: (player) => {
    ...
    const overworldStatus = config.overworld.enabled
      ? '§aEnabled'
      : '§cDisabled';
    ...
    form.show(player).then((response) => {
      ...
      BedrockBorderUI.openDimensionSettings(
        player,
        dimensions[response.selection],
      );
    });
  },
  ...
  openDimensionSettings: async (player, dimension) => {
    ...
    form
      .textField(
        'Distance from center (chunks)',
        `Min ${DEFAULTS.MIN_BORDER_SIZE_CHUNKS}`,
        config.sizeChunks.toString(),
      )
    ...
    if (isNaN(sizeChunks) || sizeChunks < DEFAULTS.MIN_BORDER_SIZE_CHUNKS) {
      player.sendMessage(
        `${COLORS.ERROR}Invalid size. Must be at least ${DEFAULTS.MIN_BORDER_SIZE_CHUNKS} chunk(s)`,
      );
      return;
    }
    ...
    player.sendMessage(
      `${COLORS.SUCCESS}Settings updated for ${dimensionName}:`,
    );
    player.sendMessage(
      `${COLORS.WARNING}Border: ${statusText} ${COLORS.NEUTRAL}| Size: ${COLORS.INFO}${sizeChunks} chunks ${COLORS.NEUTRAL}(${sizeBlocks} blocks)`,
    );
    player.sendMessage(
      `${COLORS.WARNING}Center: ${COLORS.INFO}${centerX}, ${centerZ}`,
    );
  },
};
