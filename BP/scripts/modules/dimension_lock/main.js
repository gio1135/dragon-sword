...
world.afterEvents.playerDimensionChange.subscribe((event) => {
  ...
  if (toDimension.id === DIMENSIONS.NETHER && isLocked(SETTINGS.LOCK_NETHER)) {
    ...
    toDimension.id === DIMENSIONS.THE_END &&
    isLocked(SETTINGS.LOCK_END)
  ) {
    locked = true;
    ...
    for (let y = -1; y <= 5; y++) {
      for (let z = -4; z <= 4; z++) {
        ...
        try {
          ...
          if (
            b.typeId === 'minecraft:obsidian' ||
            (b.typeId === 'minecraft:netherrack' && y <= 0)
          ) {
  ...
  if (block.typeId === 'minecraft:obsidian') {
    if (
      ...
      if (isLocked(SETTINGS.LOCK_NETHER)) {
        event.cancel = true;
        system.run(() =>
          player.sendMessage(Lang.translate('chat.locked:nether')),
        );
      }
    }
  }
  ...
  if (block.typeId === 'minecraft:end_portal_frame') {
    if (itemStack.typeId === 'minecraft:ender_eye') {
      if (isLocked(SETTINGS.LOCK_END)) {
        event.cancel = true;
        system.run(() =>
          player.sendMessage(Lang.translate('chat.locked:the_end')),
        );
      }
    }
  }
