  ...
  for (const player of players) {
    ...
    if (playerData.firstPoint && playerData.resizingClaimName === '') {
      ...
      for (let yOffset = 0; yOffset < 3; yOffset += 0.5) {
        try {
          player.spawnParticle('minecraft:end_rod', {
            x: fP.x + 0.5,
            y: fP.y + 0.5 + yOffset,
            z: fP.z + 0.5,
          });
      ...
      const drawRect = (x, z, w, h) => {
        for (let i = 0; i < w; i++) {
          try {
            player.spawnParticle(particle, { x: x + i + 0.5, y, z: z + 0.5 });
          } catch (e) {}
          try {
        ...
        for (let i = 0; i < h; i++) {
          try {
            player.spawnParticle(particle, { x: x + 0.5, y, z: z + i + 0.5 });
          } catch (e) {}
          try {
            player.spawnParticle(particle, {
              x: x + w - 1 + 0.5,
    ...
    if (playerData.firstPoint && playerData.resizingClaimName !== '') {
      ...
      const dz = previewZ >= anchor.z ? -1 : 1;
      ...
      for (let i = 0; i < minSize; i++) {
        ...
        try {
          player.spawnParticle(particle, { x: px + 0.5, y, z: previewZ + 0.5 });
        } catch (e) {}
      }
      ...
      for (let i = 0; i < minSize; i++) {
        ...
        try {
          player.spawnParticle(particle, { x: previewX + 0.5, y, z: pz + 0.5 });
        } catch (e) {}
      }
    }
    ...
    ClaimManager.runInAllClaims((claim) => {
      ...
      const distSquared =
        Math.pow(pLoc.x - claimCenter.x, 2) +
        Math.pow(pLoc.z - claimCenter.z, 2);
      if (distSquared > 4096) return;
...
function showClaimParticles(player, claim, pLoc, claimShovelOut) {
  ...
  const canEnter = claim.hasPermission(
...
function renderClaimBox(player, minX, minZ, maxX, maxZ, pLoc, particleType) {
  ...
  const headSubchunkY =
    Math.floor((pLoc.y + 1.62) / subchunkHeight) * subchunkHeight;
  ...
  const spawnLine = (x1, z1, x2, z2, y) => {
    ...
    const distToLine = Math.sqrt(
      Math.pow(midX - pLoc.x, 2) + Math.pow(midZ - pLoc.z, 2),
    );
