...
export class StatsManager {
  ...
  onPlayerJoin(player) {
    ...
    try {
      ...
      console.warn(
        `[StatsManager] failed to register player in world registry: ${error.message}`,
      );
    }
  }
  ...
  onPlayerLeave(player) {
    try {
      ...
      console.warn(
        `[StatsManager] failed to save world snapshot on leave: ${error.message}`,
      );
    }
  ...
  flushAllDirty() {
    for (const player of world.getAllPlayers()) {
      try {
        ...
        console.warn(
          `[StatsManager] failed to flush stats for ${player.name}: ${error.message}`,
        );
      }
    }
  }
  ...
  adminResetPlayer(playerId, playerName) {
    try {
      ...
      if (onlinePlayer) {
        ...
        WorldStorageAdapter.savePlayerSnapshot(
          playerId,
          playerName,
          freshStats,
        );
      ...
      console.warn(
        `[StatsManager] failed to admin reset player ${playerName}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  syncToWorldStorage() {
    for (const player of world.getAllPlayers()) {
      try {
        ...
        console.warn(
          `[StatsManager] failed to sync world snapshot for ${player.name}: ${error.message}`,
        );
      }
    }
  }
