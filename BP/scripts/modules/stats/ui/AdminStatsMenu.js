...
export async function showAdminStatsMenu(player) {
  ...
  const form = new ActionFormData()
    .title('§l§6Player statistics§r')
    .body(
      `§7Select a player to view their statistics\n§8${players.length} player(s) found§r`,
    );
  ...
  try {
    ...
    if (response.selection === 0) {
      runMigration();
      player.sendMessage(
        '§aMigration process started check content log for details§r',
      );
    ...
    if (selectedPlayer) {
      const stats = WorldStorageAdapter.getPlayerStats(
        selectedPlayer.id,
        selectedPlayer.name,
      );
      if (stats) {
        await showPlayerAdminMenu(
          player,
          selectedPlayer.name,
          selectedPlayer.id,
          stats,
...
async function showPlayerAdminMenu(
  adminPlayer,
  targetName,
  targetId,
  stats,
  ...
  try {
    ...
    if (success) {
      adminPlayer.sendMessage(
        `§aSuccessfully reset statistics for §6${targetName}§r`,
      );
    } else {
      adminPlayer.sendMessage(
        `§cFailed to reset statistics for §6${targetName}§r`,
      );
    }
