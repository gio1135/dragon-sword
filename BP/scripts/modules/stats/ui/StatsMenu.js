...
const PAGES = [
  {
    id: 'general',
    title: '§6General§r',
    subtitle: '§8Time played, sessions, xp, deaths§r',
    renderer: StatsRenderer.renderGeneral,
  },
  {
    id: 'combat',
    title: '§cCombat§r',
    subtitle: '§8Kills, damage, deaths§r',
...
async function showMainMenu(player, stats, displayName) {
  const form = new ActionFormData().title(`§l§6${displayName}'s stats`);
...
async function showDetailPage(player, stats, page, displayName) {
  ...
  const form = new ActionFormData()
    .title(
      `§l${page.id.charAt(0).toUpperCase() + page.id.slice(1)} - detailed§r`,
    )
    .body(content);
