...
import { CheckCanHarvest, GetBlockCategory } from './../requirements.js';
import { IdToPos, DisplayActionBar } from './../utils.js';
import { highlightManager } from './../highlight.js';
import { CheckModeActive, ValidateMining } from './validation_handler.js';
...
export function RunChainBreak2(
  ...
  ProcessMiningStep(player, blockList, targetTypeId, magnetPos, initialCount);
}
...
export function ProcessMiningStep(
  ...
  system.run(() => {
    ...
    for (let i = 0; i < blocksToBreak; i++) {
      ...
      ExecuteSingleBreak(player, blockList, targetTypeId, magnetPos);
...
export function ExecuteSingleBreak(player, blockList, targetTypeId, magnetPos) {
  const session = highlightManager.sessions.get(player.id);
  ...
  if (!DS.claims.hasPermission(currentBlockPos, player, 'breakBlocks')) {
    return StopMining(player, session);
  }
