...
export function SpawnLaser(player, targetPos) {
  ...
  const steps = Math.min(50, Math.max(1, Math.floor(distance * 3)));
  ...
  dim.spawnParticle('minecraft:sonic_explosion', endPos);
}

export function ProcessMiningStepLazer(
  player,
  blockList,
  targetTypeId,
  magnetPos,
  ...
  system.run(() => {
    ...
    SpawnLaser(player, centerPos);

    player.dimension.runCommandAsync(
      `playsound random.orb @a ${centerPos.x} ${centerPos.y} ${centerPos.z} 0.6 2`,
    );
    ...
    for (let i = 0; i < clusterSize; i++) {
      ...
      ExecuteSingleBreak(player, blockList, targetTypeId, magnetPos, settings);
    }
