...
export function ApplyExhaustion(player, amount) {
  addExhaustion(player, amount);
}

export function ApplyMassExhaustion(player, count, multiplier = 1) {
  addExhaustion(player, count * 0.025 * multiplier);
}

export function CheckHunger(player) {
  const hungerComp = player.getComponent('minecraft:player.hunger');
  if (hungerComp && hungerComp.currentValue <= 6) return false;
  return true;
}