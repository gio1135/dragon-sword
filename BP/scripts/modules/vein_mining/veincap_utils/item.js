...
function reduceDurability(player, item) {
  ...
  if (enchantComp) {
    const unbreaking =
      enchantComp.getEnchantment('unbreaking') ||
      enchantComp.getEnchantment('minecraft:unbreaking');
...
function getToolLevel(item) {
  ...
  if (item.typeId.includes('wooden_')) {
    ...
    item.typeId.includes('stone_') ||
    item.typeId.includes('copper_')
  ) {
    level = 2;
  } else if (
    item.typeId.includes('iron_') ||
    item.typeId === 'better_on_bedrock:amethyst_pickaxe'
  ) {
    level = 3;
...
export { calculateFortune, getToolLevel, reduceDurability, spawnItem };
